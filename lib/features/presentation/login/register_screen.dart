            ...
            children: [
              ...
              Padding(
                ...
                child: SizedBox(
                  height: 50.0,
                  child: TextField(
                    controller: loginProvider.companyCode,
              ...
              GestureDetector(
                onTap: () async {
                  if (loginProvider.registerUserName.text.trim().isNotEmpty &&
                      loginProvider.companyCode.text.trim().isNotEmpty) {
                    if (_connectionStatus[0] == ConnectivityResult.mobile ||
                        _connectionStatus[0] == ConnectivityResult.wifi) {
                      RegisterResponseModel response = await registerApiRequest(
                          loginProvider.registerUserName.text,
                          loginProvider.companyCode.text);
                    ...
                    ScaffoldMessenger.of(context)
                        .showSnackBar(const SnackBar(content: Text(userCode)));
                  }
                },
                child: Padding(
                  padding: const EdgeInsets.all(8.0),
